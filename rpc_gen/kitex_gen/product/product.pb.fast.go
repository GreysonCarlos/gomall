...
func (x *Product) fastReadField6(buf []byte, _type int8) (offset int, err error) {
	...
	x.Catagories = append(x.Catagories, v)
	return offset, err
}
...
func (x *Product) fastWriteField6(buf []byte) (offset int) {
	if len(x.Catagories) == 0 {
		return offset
	}
	for i := range x.GetCatagories() {
		offset += fastpb.WriteString(buf[offset:], 6, x.GetCatagories()[i])
	}
	return offset
}
...
func (x *Product) sizeField6() (n int) {
	...
	for i := range x.GetCatagories() {
		n += fastpb.SizeString(6, x.GetCatagories()[i])
	}
	return n
}
